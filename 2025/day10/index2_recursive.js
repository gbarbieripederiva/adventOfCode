...
for (const m of machines) {
  ...
  function algo(presses) {
    let firstNullIndex = presses.findIndex((v) => v == null);
    let state = m.buttons.reduce((a, b, i) => {
      return applyButton(a, b, presses[i] ?? 0);
    }, new Array(m.joltage.length).fill(0));
    ...
    if (firstNullIndex == presses.length - 1) {
      ...
      if (solvedState(state)) {
        return presses.reduce((a, c) => a + (c ?? 0), max);
      } else {
        return null;
      }
    } else {
      return valuesForButton(state, nextB)
        .map((v) => algo(presses.with(firstNullIndex, v)))
        .reduce(
          (a, c) => (c == null ? a : a == null ? c : c < a ? c : a),
          null
        );
    }
  }

  res += algo(Array(m.buttons.length).fill(null));
}
