...
let stack = [
  ...findDevice("svr").outputs.map((v) => ({ device: v, path: [v] })),
];
let pathOut = [];
while (stack.length > 0) {
  let next = stack.pop();
  let d = findDevice(next.device);
  if ((d?.outputs?.length ?? 0) > 0) {
    stack.push(
      ...d.outputs.map((v) => ({ device: v, path: [...next.path, v] }))
    );
  } else if (next.device == "out") {
    pathOut.push(next);
  }
}

res = pathOut.filter(
  (v) => v.path.includes("dac") && v.path.includes("fft")
).length;
